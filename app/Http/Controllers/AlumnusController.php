...
class AlumnusController extends Controller
{
    ...
    {
        $academicYear = AcademicYear::whereHas(
                                        'studentStatus', function($q) {
                                            $q->where('status','Lulus');
                                        }
                                    )
                                    ->get();
        $status       = StudentStatus::where('status','Lulus')->get();

        $studyProgram = StudyProgram::where('kd_jurusan',setting('app_department_id'))->get();

        return view('alumnus.attainment.index',compact(['studyProgram']));
    }
